                ...
                <div class="col-sm-24">
                    <div class="breadcrumbs">
                        <ul>
                            ...
                            <li class="product"> <strong><?php echo $brandeName?></strong></li>
                        </ul>
                    </div>
                </div>
            </div>
                            ...
                            <div class="category-products">
                                <div id="em-grid-mode" class="emcatalog-desktop-4">
                                    <ul class="tablist" style="margin-left:10px">
                                        ...
                                        <li class="tabclick <?php if(isset($index_tab_sort) && $index_tab_sort == 'product_new_desc') echo 'selected'?>"><a href="<?php echo base_url('brande-'.$currentId.'.html')?>?sortlist=product_new_desc<?php if(isset($params_sort)) echo '&'.http_build_query($params_sort)?>" title="Mới nhất">Mới nhất</a></li>
                                        <li class="tabclick <?php if(isset($index_tab_sort) && $index_tab_sort == 'product_sale_desc') echo 'selected'?>"><a href="<?php echo base_url('brande-'.$currentId.'.html')?>?sortlist=product_sale_desc<?php if(isset($params_sort)) echo '&'.http_build_query($params_sort)?>" title="Hot nhất">Hot nhất</a></li>
                                        <li class="tabclick <?php if(isset($index_tab_sort) && $index_tab_sort == 'product_saleoff_desc') echo 'selected'?>"><a href="<?php echo base_url('brande-'.$currentId.'.html')?>?sortlist=product_saleoff_desc<?php if(isset($params_sort)) echo '&'.http_build_query($params_sort)?>" title="Rẻ nhất">Rẻ nhất</a></li>
                                        <li class="tabclick <?php if(isset($index_tab_sort) && $index_tab_sort == 'product_sale_buy') echo 'selected'?>"><a href="<?php echo base_url('brande-'.$currentId.'.html')?>?sortlist=product_sale_buy<?php if(isset($params_sort)) echo '&'.http_build_query($params_sort)?>" title="Hàng đẹp">Hàng đẹp</a></li>
                                        <li class="tabclick <?php if(isset($index_tab_sort) && $index_tab_sort == 'product_view_desc') echo 'selected'?>"><a href="<?php echo base_url('brande-'.$currentId.'.html')?>?sortlist=product_view_desc<?php if(isset($params_sort)) echo '&'.http_build_query($params_sort)?>" title="Yêu thích">Yêu thích</a></li>
                                    </ul>    
                                ...
                                <div class="block-content">
                                    <dl id="narrow-by-list">
                                        <dd>
                                            <ol>
                                                ...
                                                foreach ($this->temp['temp'] as $key => $value) {
                                                    if ($value['t_type'] == T_BRANDE) {
                                                        $s = $currentId == $value['id'] ? 'selected' : '';
                                                        echo '<li><a class="'.$s.'" href="'.base_url('brande-'.$value['id']).'.html">'.$value['t_value'].'</a></li>';
                                                    }
                                                }
                                                ?>
                                            </ol>
                                        ...
                                        <?php if(isset($params_sort) && $params_sort) {
                                            ...
                                            $url_color_params = base_url('brande-'.$currentId.'.html').'?'.http_build_query($params_color);

                                            $params_size = isset($params) ? $params : array();
                                            if (isset($params['size'])) unset($params_size['size']);
                                            $url_size_params = base_url('brande-'.$currentId.'.html').'?'.http_build_query($params_size);

                                            $params_material = isset($params) ? $params : array();
                                            if (isset($params['material'])) unset($params_material['material']);
                                            $url_material_params = base_url('brande-'.$currentId.'.html').'?'.http_build_query($params_material);

                                            $params_price = isset($params) ? $params : array();
                                            if (isset($params['price'])) unset($params_price['price']);
                                            $url_price_params = base_url('brande-'.$currentId.'.html').'?'.http_build_query($params_price);
                                        ?>
                                        <dt> Bạn đã chọn</dt>
                                        <dd>
                                            <ol>
                                                            ...
                                                            <?php
                                                                ...
                                                                foreach ($this->temp['temp'] as $temp) {
                                                                    if ($temp['t_type'] == T_COLOR) {
                                                                        echo '<option value="'.base_url('brande-'.$currentId.'.html').'?color='.$temp['id'].$url_params.'">'.$temp['t_value'].'</option>';
                                                                    }
                                                                }
                                                            ?>
                                                        </select>
                                                            ...
                                                            <?php
                                                                ...
                                                                foreach ($this->temp['temp'] as $temp) {
                                                                    if ($temp['t_type'] == T_SIZE) {
                                                                        echo '<option value="'.base_url('brande-'.$currentId.'.html').'?size='.$temp['id'].$url_params.'">'.$temp['t_value'].'</option>';
                                                                    }
                                                                }
                                                            ?>
                                                        </select>
                                                            ...
                                                            <?php
                                                                ...
                                                                foreach ($this->temp['temp'] as $temp) {
                                                                    if ($temp['t_type'] == T_MATERIAL) {
                                                                        echo '<option value="'.base_url('brande-'.$currentId.'.html').'?material='.$temp['id'].$url_params.'">'.$temp['t_value'].'</option>';
                                                                    }
                                                                }
                                                            ?>
                                                        </select>
                                                            ...
                                                            <?php
                                                                ...
                                                                foreach ($this->temp['temp'] as $temp) {
                                                                    if ($temp['t_type'] == T_PRICE) {
                                                                        echo '<option value="'.base_url('brande-'.$currentId.'.html').'?price='.$temp['id'].$url_params.'">'.$temp['t_value'].'</option>';
                                                                    }
                                                                }
                                                            ?>
                                                        </select>
